 ...
 * @author bolke
 */
public class ResultSetHelper extends au.com.bytecode.opencsv.ResultSetHelperService {
    ...
    public ResultSetHelper(HashSet maskColumns) {
    ...
    private String getColumnValue(ResultSet rs, int colType, int colIndex)
		...
		if (value == null)
		{
			value = "";
                ...
                if (maskColumns.contains(rs.getMetaData().getColumnName(colIndex))) {
                    try {
                        value = SHA256.hash(value);
                    } catch (NoSuchAlgorithmException ex) {
                        Logger.getLogger(ResultSetHelper.class.getName()).log(Level.SEVERE, null, ex);
